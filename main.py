...
def index():
	...
	auth_manager = spotipy.oauth2.SpotifyOAuth(client_id=os.getenv("SPOTIPY_CLIENT_ID"), client_secret=os.getenv("SPOTIPY_CLIENT_SECRET"), redirect_uri=os.getenv(
		"SPOTIPY_REDIRECT_URI"), scope='playlist-modify-public playlist-modify-private playlist-read-private', cache_handler=cache_handler, show_dialog=True)
...
def all_playlists():
	...
	playlists = [playlist for playlist in spotify.current_user_playlists(
	)['items'] if playlist['owner']['id'] == current_user.id]

	playlists = [Playlist().from_dict(playlist) for playlist in playlists]

	return render_template("playlists.html", current_user=current_user, playlists=playlists)
...
def playlist(playlist_id):
	...
	current_user = get_current_user(spotify)
	try:
		playlist = Playlist().from_dict(data=spotify.user_playlist(current_user.id, playlist_id, fields=','.join(PLAYLIST_FIELDS.values())))
		...
		response = spotify.playlist_items(
			playlist_id,
			fields=','.join(
				['items.'+field for field in TRACK_FIELDS.values()]+
				["total"]
			))
...
def change_order():
	...
	try:
		playlist = Playlist().from_dict(data=spotify.user_playlist(
