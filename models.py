...
class Track(BaseModel):
    ...
    def from_dict(self, data: dict):
        ...
        self.added_by = User().from_dict(
            {'id': track[TRACK_FIELDS['added_by']]})
        self.artists = [v for k, v in track.items(
...
class Playlist(BaseModel):
    ...
    def from_dict(self, data: dict):
        ...
        self.owner = User().from_dict(
            {'id': playlist[PLAYLIST_FIELDS['owner']]})
    ...
    def __str__(self):
        return f'{self.name} - {(self.description[:40] + "..") if len(self.description) > 75 else self.description}' if len(self.description) else str(self.name)
